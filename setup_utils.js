...
let capturedShipRespawnX_NormalMode = 0; // <<< TOEGEVOEGD: X-positie voor respawn na capture (Normal modes)
...
// --- START OF FILE setup_utils.js ---
// --- DEEL 2      van 3 dit code blok    --- (<<< REVISE: Sound volume settings aligned vertically >>>)
// <<< REVISE: AI_SMOOTHING_FACTOR_MOVE verlaagd, AI_MOVEMENT_DEADZONE verhoogd om trillen te verminderen >>>
const
    // <<< START WIJZIGING: Difficulty Scaling Constants (aangepast in deel 1) >>>
    ...
    // <<< EINDE WIJZIGING: Difficulty Scaling Constants (aangepast in deel 1) >>>
    ...
    AI_SMOOTHING_FACTOR_MOVE = 0.05, // <<< GEWIJZIGD (was bijv. 0.08 of 0.1)
    ...
    AI_MOVEMENT_DEADZONE = 0.8, // <<< GEWIJZIGD (was bijv. 0.1)
    ...
    CS_BONUS_MESSAGE_DURATION = 5000,
    CS_CLEAR_DELAY = 8000,
...
// --- START OF FILE setup_utils.js ---
// --- DEEL 3      van 3 dit code blok    --- (<<< REVISE: Skip only "GAME OVER" text in 2P final sequence >>>)
// <<< REVISE: Pauzeerfunctie (P-toets & R1) werkt nu ook in AI demo modes >>>
 ...
 * <<< GEWIJZIGD: Gebruikt nu currentVerticalSpacing voor Y berekening. >>>
*/
...
function handleKeyDown(e) {
    try {
        ...
        if (isInGameState) {
            // Pauzeren (P-toets) moet altijd werken in-game, ook tijdens AI demo.
            if ((e.key === 'p' || e.key === 'P') && gameOverSequenceStartTime === 0 && !isShowingPlayerGameOverMessage) {
                if(typeof togglePause === 'function') togglePause(); 
                return; // Voorkom verdere inputverwerking als gepauzeerd wordt.
            }

            if (!isPaused) {
                if (!isManualControl) { // AI Demo (1P of CO-OP)
                    // Alleen Exit (Escape/Enter) of andere toets voor AI demo stop.
                    if (e.key === "Escape" || e.key === "Enter") { 
                        if(typeof stopGameAndShowMenu === 'function') stopGameAndShowMenu(); 
                    } else if (!e.altKey && !e.ctrlKey && !e.metaKey && !e.shiftKey && e.key !== 'p' && e.key !== 'P') { // Voorkom P als stop-key
        ...
        } else { // Niet in game (menu of scorescherm)
            ...
            } else if (!isShowingScoreScreen) { // Menu input
                stopAutoDemoTimer();
                switch (e.key) {
...
function togglePause() {
    ...
    if (isInGameState && gameOverSequenceStartTime === 0 && !isAnyGameOverMessageShowing) {
        if (!isManualControl) { // Demo modes (1P AI of CO-OP AI)
            canPause = true; // Demo's kunnen altijd pauzeren als ze in game state zijn
        } else if (isTwoPlayerMode && selectedGameMode === 'coop') {
            // In manual CO-OP, pauzeer als ten minste één speler actief is (niet gevangen & levens)
