...
function resetGameInternal() { 
    ...
    if (isTwoPlayerMode) {
        ...
        player1CaptureRespawnX = 0; // <<< TOEGEVOEGD: Reset
        player2CaptureRespawnX = 0; // <<< TOEGEVOEGD: Reset
        player1NeedsRespawnAfterCapture = false; // <<< TOEGEVOEGD: Reset
        player2NeedsRespawnAfterCapture = false; // <<< TOEGEVOEGD: Reset
 ...
 * <<< CO-OP: De targeting logic is nog gebaseerd op het globale `ship` object voor 1P/Alternating.
 * Voor CO-OP, kiest het een van de actieve schepen. >>>
 */
...
// ... (andere functies zoals renderExplosions, drawCanvasButton etc. blijven hier) ...

/**
 * Tekent de volledige game scène.
 * <<< GEWIJZIGD: Tekent nu ship1 en ship2 in CO-OP mode. >>>
 */
function renderGame() {
    try {
...
function handleEnemyHit(enemy, shootingPlayerId = null) { 
    ...
    if (enemy.health <= 0) { 
        ...
        if (bossHadCapturedShipInitially && bossHadDimensionsInitially) {
            if (isCoopAIDemoActive) {
                // In CO-OP AI Demo:
                // Als het een *partner* schip was (bijv. P1 was gevangen door deze boss),
                // dan moet het vallende schip targetPlayerId 'player1' krijgen.
                // Anders (neutraal schip), gaat het naar de schietende AI.
                if (wasPartnershipCapturedByThisBoss && partnerWhoWasCapturedId) {
                    fallingShipTargetPlayerId = partnerWhoWasCapturedId;
                } else {
                    // Neutraal schip, of partner-logica faalde (zou niet moeten), dus geef aan schutter.
                    fallingShipTargetPlayerId = shootingPlayerId; 
                }
            } else if (isTwoPlayerMode && selectedGameMode === 'coop') {
                // Handmatige CO-OP (logica blijft zoals het was):
...
// <<< CO-OP AI DEMO REVISE: Agressievere kogelontwijking in calculateAIDesiredState. >>>
// <<< CO-OP AI DEMO REVISE: Zorg dat AI schiet op boss die partner gevangen houdt, zelfs als partner game over message toont. >>>
// <<< CO-OP AI DEMO REVISE: AI met 1 leven vliegt niet in capture beam. >>>
// <<< CO-OP AI DEMO REVISE: AI beschouwt nu elke boss met een gevangen schip als potentieel doelwit. >>>
// <<< CO-OP AI DEMO REVISE: AI met 1 leven benadert niet actief een boss met een *neutraal* gevangen schip. >>>
// <<< CO-OP AI DEMO REVISE: Dual ship AI houdt vuur op "schone" boss als partner dual ship nodig heeft. >>>
// <<< CO-OP AI DEMO REVISE: Alleen AI die gevangen was (of schutter van neutrale) probeert vallend schip te vangen. >>>

/**
 * Fires a player bullet if game state permits.
 * Kan nu optioneel een `shooterId` ('player1' of 'player2') accepteren voor CO-OP.
 * Voor 1P/2P-Normal modes, `isWaitingForRespawn` en `isInvincible` blokkeren het vuren NIET.
 */
function firePlayerBullet(shooterId = null) {
    ...
    if (isCoopAIDemoActive && shooterId === null) { 
        return false;
    }
    ...
    const baseChecks = !isPaused && isInGameState &&
                       gameOverSequenceStartTime === 0 && !isShowingPlayerGameOverMessage && // Globale game over check
                       ...
                       !(isTwoPlayerMode && selectedGameMode === 'coop' && 
                         ((shooterId === 'player1' && isPlayer1ShowingGameOverMessage) || // Specifiek voor de schutter
                          (shooterId === 'player2' && isPlayer2ShowingGameOverMessage)));

    if (!baseChecks) return false;

    if (isTwoPlayerMode && selectedGameMode === 'coop' && !isCoopAIDemoActive) { 
        if (shooterId === 'player1' && ship1 && player1Lives > 0 && !isPlayer1ShipCaptured && !player1NeedsRespawnAfterCapture) { 
            if (!isShowingCaptureMessage || (isShowingCaptureMessage && !isPlayer1ShipCaptured)) { 
                ...
                canShootLogic = true; currentShooterPlayerId = 'player1';
            }
             ...
             if (!isShowingCaptureMessage || (isShowingCaptureMessage && !isPlayer2ShipCaptured)) { 
                ...
                canShootLogic = true; currentShooterPlayerId = 'player2';
            }
        }
        ...
        if (ship && playerLives > 0 && !isShipCaptured) { 
            ...
            if (isTwoPlayerMode && selectedGameMode === 'normal') { 
                if (currentPlayer === 1) {
                    ...
                    getSingleShotFlag = () => p1JustFiredSingle; setSingleShotFlag = (val) => { p1JustFiredSingle = val; }; currentShooterPlayerId = 'player1';
                } else { 
                    ...
                    getSingleShotFlag = () => p2JustFiredSingle; setSingleShotFlag = (val) => { p2JustFiredSingle = val; }; currentShooterPlayerId = 'player2';
                }
            } else { 
    ...
    if (!useSingleShotFlag && (now - playerSpecificLastShotTime < SHOOT_COOLDOWN)) return false;
    if (useSingleShotFlag && getSingleShotFlag()) return false; 
     ...
     try {
        ...
        if (connectedGamepadIndex !== null) {
            ...
            if (gamepads?.[connectedGamepadIndex]) {
                const gamepadP1 = gamepads[connectedGamepadIndex];
                const ctrlResultP1 = processSingleController(gamepadP1, previousGameButtonStates);
                controllerP1Left = ctrlResultP1.left; controllerP1Right = ctrlResultP1.right;
                let p1CanShootViaController = !isShowingIntro && !inCoopLevel1IntroStrict && ctrlResultP1.shoot;

                if (isShowingCaptureMessage) {
                    if (isTwoPlayerMode && selectedGameMode === 'coop' && isPlayer1ShipCaptured) p1CanShootViaController = false;
                    else if (!(isTwoPlayerMode && selectedGameMode === 'coop') && isShipCaptured) p1CanShootViaController = false;
                }
                controllerP1ShootIsDown = p1CanShootViaController;
                previousGameButtonStates = ctrlResultP1.newButtonStates.slice();

                if (ctrlResultP1.pause && !isShowingCaptureMessage) { togglePause(); return; } 
                if (ctrlResultP1.back && !isShowingCaptureMessage) { stopGameAndShowMenu(); return; } 
            } else { if (previousGameButtonStates.length > 0) previousGameButtonStates = []; }
        } else { if (previousGameButtonStates.length > 0) previousGameButtonStates = []; }

        if (isTwoPlayerMode && connectedGamepadIndexP2 !== null) {
            const gamepads = navigator.getGamepads();
            if (gamepads?.[connectedGamepadIndexP2]) {
                const gamepadP2 = gamepads[connectedGamepadIndexP2];
                const ctrlResultP2 = processSingleController(gamepadP2, previousGameButtonStatesP2);
                controllerP2Left = ctrlResultP2.left; controllerP2Right = ctrlResultP2.right;
                let p2CanShootViaController = !isShowingIntro && !inCoopLevel1IntroStrict && ctrlResultP2.shoot;
                 if (isShowingCaptureMessage) {
                    if (isTwoPlayerMode && selectedGameMode === 'coop' && isPlayer2ShipCaptured) p2CanShootViaController = false;
                 }
                controllerP2ShootIsDown = p2CanShootViaController;
                previousGameButtonStatesP2 = ctrlResultP2.newButtonStates.slice();
            } else { if (previousGameButtonStatesP2.length > 0) previousGameButtonStatesP2 = []; }
        } else { if (previousGameButtonStatesP2.length > 0) previousGameButtonStatesP2 = []; }

        if (isTwoPlayerMode && selectedGameMode === 'coop') { 
            const p1_can_control_coop = ship1 && player1Lives > 0 && !isPlayer1ShipCaptured && !isPlayer1WaitingForRespawn && !isPlayer1ShowingGameOverMessage && !player1NeedsRespawnAfterCapture;
            if (p1_can_control_coop) {
                leftPressed = keyboardP1LeftDown || controllerP1Left;
                rightPressed = keyboardP1RightDown || controllerP1Right;
                shootPressed = !inCoopLevel1IntroStrict && (keyboardP1ShootDown || controllerP1ShootIsDown);
                if (isShowingCaptureMessage && isPlayer1ShipCaptured) shootPressed = false;
            } else {
                leftPressed = false; rightPressed = false; shootPressed = false;
            }
            const p2_can_control_coop = ship2 && player2Lives > 0 && !isPlayer2ShipCaptured && !isPlayer2WaitingForRespawn && !isPlayer2ShowingGameOverMessage && !player2NeedsRespawnAfterCapture;
            if (p2_can_control_coop) {
                p2LeftPressed = keyboardP2LeftDown || controllerP2Left;
                p2RightPressed = keyboardP2RightDown || controllerP2Right;
                p2ShootPressed = !inCoopLevel1IntroStrict && (keyboardP2ShootDown || controllerP2ShootIsDown);
                if (isShowingCaptureMessage && isPlayer2ShipCaptured) p2ShootPressed = false;
            } else {
                p2LeftPressed = false; p2RightPressed = false; p2ShootPressed = false;
            }
        } else if (isTwoPlayerMode && selectedGameMode === 'normal' && currentPlayer === 2) {
            const p2_alt_can_control = ship && playerLives > 0 && !isShipCaptured && !isShowingPlayerGameOverMessage;
            if (p2_alt_can_control) {
                leftPressed = keyboardP2LeftDown || controllerP2Left;
                rightPressed = keyboardP2RightDown || controllerP2Right;
                shootPressed = !isShowingIntro && (keyboardP2ShootDown || controllerP2ShootIsDown);
            ...
            const p1_can_control_normal = ship && playerLives > 0 && !isShipCaptured && !isShowingPlayerGameOverMessage;
            if (p1_can_control_normal) {
                leftPressed = keyboardP1LeftDown || controllerP1Left;
                rightPressed = keyboardP1RightDown || controllerP1Right;
                shootPressed = !isShowingIntro && (keyboardP1ShootDown || controllerP1ShootIsDown); 
                if (isShowingCaptureMessage && isShipCaptured) shootPressed = false;
            } else {
                leftPressed = false; rightPressed = false; shootPressed = false;
            }
        ...
        if (isTwoPlayerMode && selectedGameMode === 'coop' && !isCoopAIDemoActive) {
            if (ship1 && player1Lives > 0 && !isPlayer1ShipCaptured && !isPlayer1WaitingForRespawn && !isPlayer1ShowingGameOverMessage && !player1NeedsRespawnAfterCapture) {
                if (shootPressed) {
                    if (isSingleShotMode) { if (!p1FireInputWasDown) firePlayerBullet('player1'); }
                    else { firePlayerBullet('player1'); }
                }
            }
            if (ship2 && player2Lives > 0 && !isPlayer2ShipCaptured && !isPlayer2WaitingForRespawn && !isPlayer2ShowingGameOverMessage && !player2NeedsRespawnAfterCapture) {
                 if (p2ShootPressed) {
                    if (isSingleShotMode) { if (!p2FireInputWasDown) firePlayerBullet('player2'); }
                    else { firePlayerBullet('player2'); }
                }
            }
        } else if (!isCoopAIDemoActive) { 
            if (ship && playerLives > 0 && !isShipCaptured && !isShowingPlayerGameOverMessage) {
                const activePlayerIdForFiring = (isTwoPlayerMode && selectedGameMode === 'normal' && currentPlayer === 2) ? 'player2' : 'player1';
                const useP1FireInputWasDown = (isTwoPlayerMode && selectedGameMode === 'normal' && currentPlayer === 1) || !isTwoPlayerMode;

                if (shootPressed) {
                    if (isSingleShotMode) {
                        if (useP1FireInputWasDown && !p1FireInputWasDown) firePlayerBullet(activePlayerIdForFiring);
                        else if (!useP1FireInputWasDown && !p2FireInputWasDown) firePlayerBullet(activePlayerIdForFiring);
                    } else {
                        firePlayerBullet(activePlayerIdForFiring);
                    }
                }
            }
        }
        ...
        if (isTwoPlayerMode && selectedGameMode === 'coop' && !isCoopAIDemoActive) {
             p1FireInputWasDown = (keyboardP1ShootDown || controllerP1ShootIsDown);
             p2FireInputWasDown = (keyboardP2ShootDown || controllerP2ShootIsDown);
...
/**
 * Controls player ship via AI (demo mode). Geldt alleen voor de standaard 1P AI Demo.
 */
 function aiControl() {
    try {
...
function aiControlCoop() {
    ...
    const isShowingBlockingMessageForCoop = showReadyMessage || showCSClearMessage || showCsHitsMessage || showPerfectMessage || showCsBonusScoreMessage || showExtraLifeMessage || isCsCompletionDelayActive || 
    ...
    if (p1CanAct && ship1) {
        ...
        const p1Blocking = isShowingBlockingMessageForCoop || isPlayer1ShowingGameOverMessage;
        let { desiredTargetX: dt1, shouldTryShoot: sts1, targetEnemyForAI: te1 } = calculateAIDesiredState(ship1, smoothedShip1X, player1IsDualShipActive, enemies, enemyBullets, fallingShips, isPlayer1Invincible, isPlayer1ShipCaptured, isPlayer1WaitingForRespawn, now, canvasWidth, p1Blocking, 'p1');
    ...
    if (p2CanAct && ship2) {
        ...
        const p2Blocking = isShowingBlockingMessageForCoop || isPlayer2ShowingGameOverMessage;
        let { desiredTargetX: dt2, shouldTryShoot: sts2, targetEnemyForAI: te2 } = calculateAIDesiredState(ship2, smoothedShip2X, player2IsDualShipActive, enemies, enemyBullets, fallingShips, isPlayer2Invincible, isPlayer2ShipCaptured, isPlayer2WaitingForRespawn, now, canvasWidth, p2Blocking, 'p2');
 ...
 */
function calculateAIDesiredState(currentShip, currentSmoothedX, isShipDual, gameEnemies, gameEnemyBullets, gameFallingShips, isShipInvincible, isThisShipCaptured, isThisShipWaitingForRespawn, currentTime, gameCanvasWidth, isCurrentShipShowingBlockingMessage, shipIdentifier) {
    ...
    if (isCurrentShipShowingBlockingMessage || isThisShipCaptured || isThisShipWaitingForRespawn || (shipIdentifier === 'p1' && player1NeedsRespawnAfterCapture) || (shipIdentifier === 'p2' && player2NeedsRespawnAfterCapture) ) { 
        desiredTargetX = targetCenterShipX; 
    ...
    if (isCoopAIDemoActive && otherPlayerIsActuallyCaptured && !otherPlayerIsShowingGameOver && capturedBossIdWithMessage) {
        bossHoldingPartner = gameEnemies.find(e => e.id === capturedBossIdWithMessage && e.type === ENEMY3_TYPE && e.hasCapturedShip);
    }

    if (bossHoldingPartner && !isShipDual) { // Alleen als huidige AI GEEN dual ship heeft, want dual ship moet zelf kunnen schieten.
    ...
    // <<< GEWIJZIGD: AI probeert alleen vallend schip te vangen als targetPlayerId overeenkomt >>>
...
// <<< CO-OP REVISE: Correct Game Over afhandeling bij capture met laatste leven in handlePlayerShipCollision. >>>
// <<< CO-OP REVISE: Respawn na capture message, op vanglocatie, met directe controle. >>>
// <<< CO-OP REVISE: Toon "Player X Game Over" en speel geluid bij verlies laatste leven door capture. >>>
// <<< NORMAL MODE REVISE: Sla X-positie op bij capture voor respawn op dezelfde plek. >>>
// <<< AI DEMO REVISE: AI_MOVEMENT_DEADZONE wordt hier toegepast om trillen te verminderen. >>>
            ...
            if (isManualControl) { 
                ...
                if (isCoopAIDemoActive) { 
                    if (ship1 && player1Lives > 0 && !isPlayer1ShipCaptured && !isPlayer1WaitingForRespawn && !isPlayer1ShowingGameOverMessage && !player1NeedsRespawnAfterCapture) {
                        ...
                        // <<< GEWIJZIGD: AI_MOVEMENT_DEADZONE hier toegepast >>>
...
// <<< CO-OP AI DEMO REVISE: Centreer overgebleven AI schip direct na wave/CS voltooiing. >>>
// <<< CO-OP REVISE: Speel levelUp.mp3 tijdens "STAGE X" na CS bonus in CO-OP (demo) modes. >>>
// <<< NORMAL MODE REVISE: Gebruik opgeslagen X-positie voor respawn na capture (alleen manual). >>>
...
function runSingleGameUpdate(timestamp) {
    try {
        ...
        if (isShowingCaptureMessage) {
            ...
            if (isInGameState && !isShowingPlayerGameOverMessage && !isPlayer1ShowingGameOverMessage && !isPlayer2ShowingGameOverMessage && gameOverSequenceStartTime === 0) { if (isManualControl || isCoopAIDemoActive) handlePlayerInput(); if (typeof moveEntities === 'function') moveEntities(); updateExplosions(); updateFloatingScores(); }
            ...
            if (now - captureMessageStartTime >= CAPTURE_MESSAGE_DURATION) { 
                ...
                const shipBaseY = gameCanvas ? gameCanvas.height - SHIP_HEIGHT - SHIP_BOTTOM_MARGIN : 500;
                 ...
                 } else if (!isTwoPlayerMode && !isManualControl) { // 1P AI Demo (blijft ongewijzigd voor deze specifieke opdracht)
                 ...
                 } else if (isManualControl) { // 1P/2P Normal Manual
                    if (isShipCaptured && playerLives > 0) {
        ...
        // <<< NIEUWE LOGICA: Centreer overgebleven CO-OP AI schip voor CS bonus >>>
        if (isCoopAIDemoActive && isChallengingStage && enemies.length === 0 && !isCsCompletionDelayActive && !showCsBonusScoreMessage && !showPerfectMessage && gameCanvas && gameCanvas.width > 0) {
            ...
            // Als beide nog actief zijn, blijven ze op hun CO-OP posities, of de AI logic handelt het af.
        }
        // <<< EINDE NIEUWE LOGICA >>>
        ...
        if (inNormalIntro && noPlayerGameOverIsActive && gameOverSequenceStartTime === 0) {
            ...
            if (introStep === 1) { 
                ...
                if (!stageIntroSoundPlayed) { 
                    if (playLevelUpAfterCSBonus && (isTwoPlayerMode && selectedGameMode === 'coop')) { // <<< GEWIJZIGD: Check of het een CO-OP (demo) mode is >>>
                        ...
                        playLevelUpAfterCSBonus = false; 
                         ...
                         if (level > 1 || (level === 1 && isTwoPlayerMode && selectedGameMode === 'normal' && !playerIntroSoundPlayed && !isCoopAIDemoActive && initialGameStartSoundPlayedThisSession)) { 
                            playSound(levelUpSound);
                              ...
                              if(!initialGameStartSoundPlayedThisSession){ playSound(startSound); initialGameStartSoundPlayedThisSession = true; coopStartSoundPlayedThisSession = true; }
                         }
                    }
        ...
        if (noSpecialOrNormalIntroRunning && noPlayerGameOverIsActive && gameOverSequenceStartTime === 0) {
            ...
            if (!isWaveTransitioning && activePlayersForLogic && !inNormalIntro && !isShowingCaptureMessage && !isEntrancePhaseActive) {
                ...
                if (waveConsideredComplete) {
                    ...
                    // <<< GEWIJZIGD: Deze blok is al aanwezig, geen directe wijziging nodig voor *deze* request >>>
                    // De logica hieronder (die al bestond) zorgt voor het centreren van een *enkel overgebleven* CO-OP AI schip
                    // NA een wave/CS voltooiing, wat ook al het geval was *tijdens* de CS bonus screen.
                    // De nieuwe request was om het *voor* de bonus text te doen, wat hierboven is afgehandeld.
                    if (isCoopAIDemoActive && gameCanvas && gameCanvas.width > 0) {
                        ...
                        // Als beide nog leven, blijven ze op hun normale CO-OP posities.
                    }
