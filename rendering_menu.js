...
// <<< REVISE: Pauzeerfunctie (P-toets & R1) werkt nu ook in AI demo modes (via togglePause) >>>
...
function pollControllerForMenu() {
    try {
        ...
        // In-game acties (Pauze/Terug) hebben voorrang als het spel bezig is.
        // togglePause() en stopGameAndShowMenu() hebben hun eigen logica om te bepalen OF ze mogen uitgevoerd worden.
        ...
        // Als er geen in-game actie is genomen, verwerk menu/score screen input.
        if (!actionTakenThisFrame) {
            ...
            if (canConsiderReturningToMenu) {
                ...
                for (let i = 0; i < currentGeneralButtonStates.length; i++) {
                    if (i === PS5_BUTTON_R1 || i === PS5_BUTTON_TRIANGLE) continue; // R1/Triangle zijn voor in-game pauze/exit
...
function showMenuState() {
    try {
       ...
       aiPlayerActivelySeekingCaptureById = null; // Reset CO-OP AI capture vlag
       ...
       // Alleen proberen schip te positioneren als het daadwerkelijk bestaat en canvas gereed is.
       // Dit is vooral relevant als showMenuState wordt aangeroepen vanuit een game state.
       // Bij de allereerste initialisatie zal 'ship' nog niet correct gezet zijn door resetGame.
       ...
       } else if (ship !== null) { // Alleen loggen als ship niet expliciet null is (wat het is na een CO-OP game)
            // De console.warn blijft nuttig voor debuggen als de conditie faalt terwijl ship wel "iets" is.
            // console.warn("showMenuState: Ship not positioned - ship object invalid or canvas not ready."); // DEBUG
       }
...
function baseStartGame(setManualControl) {
    try {
        ...
        if (typeof window.resetGame === 'function') {
            window.resetGame(); // Dit zal het 'ship' object correct initialiseren.
        } else {
            ...
            if (!initialGameStartSoundPlayedThisSession) {
             ...
             if (!initialGameStartSoundPlayedThisSession) {
                if (isTwoPlayerMode && selectedGameMode === 'normal') {
                    // Voor 2P Normal L1, wordt startSound in runSingleGameUpdate afgehandeld.
                    // Zorg dat coopStartSoundPlayedThisSession niet wordt gezet zodat het daar kan spelen.
                } else { // 1P Manual L1
                    // Voor 1P Manual L1, wordt startSound ook in runSingleGameUpdate afgehandeld.
                    coopStartSoundPlayedThisSession = true; // Voorkom dat een latere CO-OP het nogmaals speelt.
                }
            }
        }
        ...
        selectedButtonIndex = -1;
        // De initiële positie van het 'ship' object wordt nu correct afgehandeld door resetGame.
        ...
        if (drawMenuShip) {
            ...
            if (isTwoPlayerMode && selectedGameMode === 'coop') { 
                ...
                if (p1ActiveAndAlive && (!isPlayer1ShipCaptured || (isPlayer1ShipCaptured && !isShowingCaptureMessage))) {
                    ...
                    // Voor CO-OP AI demo, als P1 de enige is tijdens CS bonus, wordt het door game_logic gecentreerd.
                    // Hier tekenen we het dan op die gecentreerde positie.
                    // Voor manual CO-OP, als P1 de enige is tijdens CS bonus, gebeurt centreren ook in game_logic.
                    if (isShowingCSBonusScreen && !p2ActiveAndAlive) { 
                        // P1 is de enige, teken op de (door game_logic) gecentreerde positie
                    } else if (isShowingCSBonusScreen && p1ActiveAndAlive && p2ActiveAndAlive && isCoopAIDemoActive) {
                        // Beide AI leven, teken op hun CO-OP startposities (game_logic zorgt hiervoor)
                    } else if (isShowingCSBonusScreen && isCoopAIDemoActive) { // Andere AI demo CS bonus scenario's waar P1 niet de enige overlevende is
                         shouldDrawP1 = false; 
                    } // In manual CO-OP met beide spelers, of buiten CS bonus, wordt P1 normaal getekend.
                ...
                if (p2ActiveAndAlive && (!isPlayer2ShipCaptured || (isPlayer2ShipCaptured && !isShowingCaptureMessage))) {
                    ...
                    if (isShowingCSBonusScreen && !p1ActiveAndAlive) { // P2 is de enige
                        // P2 is (of wordt) gecentreerd.
                    } else if (isShowingCSBonusScreen && p1ActiveAndAlive && p2ActiveAndAlive && isCoopAIDemoActive) {
                        // Beide AI leven.
                 ...
                 if (ship && playerLives > 0 && !isShipCaptured && !isShowingCaptureMessage) {
                    ...
                    if (isInvincible || isWaitingForRespawn) {
                     ...
                     // Voor 1P AI Demo tijdens CS bonus, wordt het schip al gecentreerd door game_logic
